...
final class LateNinthBatchAuditTests: XCTestCase {
    ...
    func testProviderReadLeavesMainActorAndUsesChunkedOwnedReadWithoutFixedLimit() throws {
        ...
        XCTAssertTrue(storage.contains("read(upToCount: chunkSize)"))
        XCTAssertFalse(storage.contains("remaining + 1"))
