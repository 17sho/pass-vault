const allowedTypes="'account','website','note','totp','custom','settings'";
function entriesCreate(columns){return`CREATE TABLE entries_new(user_id TEXT NOT NULL,id TEXT NOT NULL,type TEXT NOT NULL CHECK(type IN(${allowedTypes})),version INTEGER NOT NULL,iv TEXT NOT NULL,ciphertext TEXT NOT NULL,updated_at INTEGER NOT NULL${columns.includes('created_at')?',created_at INTEGER':''}${columns.includes('revision')?',revision INTEGER NOT NULL DEFAULT 1 CHECK(revision>=1)':''},PRIMARY KEY(user_id,id),FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE)`}
...
export function migrateEntriesForCustom(db){const sql=db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='entries'").get()?.sql||'';if(/'custom'/.test(sql))return false;return rebuildEntries(db,db.prepare("PRAGMA table_info(entries)").all().map(x=>x.name))}
...
function migrateRevisionColumn(db,table){db.exec('BEGIN IMMEDIATE');try{if(db.prepare(`PRAGMA table_info(${table})`).all().some(x=>x.name==='revision')){db.exec('COMMIT');return false}db.exec(`ALTER TABLE ${table} ADD COLUMN revision INTEGER NOT NULL DEFAULT 1 CHECK(revision>=1);COMMIT`);return true}catch(e){db.exec('ROLLBACK');throw e}}
export function migrateEntriesRevision(db){return migrateRevisionColumn(db,'entries')}
export function migrateAttachmentsRevision(db){return migrateRevisionColumn(db,'attachments')}
