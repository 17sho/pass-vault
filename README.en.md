...
The latest stable release is [v1.1.70](https://github.com/17sho/pass-vault-v2/releases/tag/v1.1.70), adding bulk pin, unpin, and move-to-Trash actions within the current filtered result scope. It includes separate `pass-vault-v2-cloudflare-1.1.70.tar.gz` / `.zip` and `pass-vault-v2-linux-1.1.70.tar.gz` / `.zip` assets plus `SHA256SUMS`. Each archive contains only its selected runtime and placeholder configuration; run `sha256sum -c SHA256SUMS` after download.
