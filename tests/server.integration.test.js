...
async function start(db, extraEnv={}) {
  child = spawn(process.execPath, ['apps/server/server.mjs'], { cwd: new URL('..', import.meta.url), env: {...process.env, INVITE_CODE:'test-invite-code-1234567890', PORT:'19876', HOST:'127.0.0.1', DB_PATH:db, ATTACHMENTS_DIR:join(db,'..','attachments'), COOKIE_SECURE:'false', CLIENT_IP_HEADER:'', ...extraEnv}, stdio:['ignore','pipe','pipe'] });
