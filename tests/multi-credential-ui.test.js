...
for(const [browserName,launcher,contextOptions] of [['Chromium',chromium,{}],['WebKit',webkit,{...devices['iPhone 13']}]] )test(`${browserName}: multi-account create/detail/edit persists without runtime errors`,async()=>{const browser=await launcher.launch({headless:true}),context=await browser.newContext(contextOptions),page=await context.newPage(),errors=[];page.on('pageerror',e=>errors.push(e.message));page.on('console',m=>{if(m.type()==='error')errors.push(m.text())});try{const username=await register(page);const editor=await openAccount(page);assert.equal(await editor.locator('.credential-row').count(),1);assert.deepEqual(await editor.locator('.credential-row legend').allTextContents(),['账号 1']);await fillBase(editor);await addRows(editor,[['alice','secret-1'],['bob','secret-2'],['carol','secret-3']]);assert.deepEqual(await editor.locator('.credential-row legend').allTextContents(),['账号 1','账号 2','账号 3']);await editor.getByRole('button',{name:'移除账号 2'}).click();assert.deepEqual(await editor.locator('.credential-row legend').allTextContents(),['账号 1','账号 2']);await saveAccount(page,editor);await page.getByText('已保存',{exact:true}).waitFor();await page.locator('.item-card',{hasText:'Multi Account'}).click();const detail=page.locator('#detail');assert.match(await detail.textContent(),/账号 1alice.*密码 1.*账号 2carol.*密码 2/s);assert.doesNotMatch(await detail.textContent(),/secret-[13]/);await detail.getByRole('button',{name:'显示密码 1'}).click();assert.match(await detail.textContent(),/secret-1/);assert.doesNotMatch(await detail.textContent(),/secret-3/);await detail.getByRole('button',{name:'显示密码 2'}).click();assert.match(await detail.textContent(),/secret-3/);await detail.getByRole('button',{name:'编辑'}).click();await editor.getByRole('button',{name:'移除账号 1'}).click();await editor.getByRole('button',{name:'+ 添加账号'}).click();await editor.locator('input[name=credentialUsername]').nth(1).fill('dave');await editor.locator('input[name=credentialPassword]').nth(1).fill('secret-4');await saveAccount(page,editor);await page.reload();await login(page,username);await page.locator('.item-card',{hasText:'Multi Account'}).click();assert.match(await page.locator('#detail').textContent(),/账号 1carol.*账号 2dave/s);assert.deepEqual(errors,[])}finally{await context.close();await browser.close()}});
