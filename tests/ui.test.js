...
test('空标签弹窗移动端保持紧凑并有独立滚动内容区',async()=>{const page=await browser.newPage({viewport:{width:390,height:844}});await register(page);await page.getByRole('button',{name:'更多',exact:true}).click();await page.getByRole('menuitem',{name:'标签',exact:true}).click();const dialog=page.locator('#tags-dialog'),metrics=await dialog.evaluate(d=>{const h=d.querySelector('.dialog-head'),list=d.querySelector('#tags-list'),title=d.querySelector('h2'),close=d.querySelector('.icon-close'),dr=d.getBoundingClientRect(),hr=h.getBoundingClientRect(),tr=title.getBoundingClientRect(),cr=close.getBoundingClientRect(),ls=getComputedStyle(list);return{height:dr.height,headLeft:hr.left-dr.left,headRight:dr.right-hr.right,titleCenter:tr.top+tr.height/2,closeCenter:cr.top+cr.height/2,listOverflow:ls.overflowY,listMinHeight:parseFloat(ls.minHeight)}});assert.ok(metrics.height<=330,JSON.stringify(metrics));assert.ok(metrics.headLeft<=1&&metrics.headRight<=1,JSON.stringify(metrics));assert.ok(Math.abs(metrics.titleCenter-metrics.closeCenter)<=12,JSON.stringify(metrics));assert.equal(metrics.listOverflow,'auto');assert.ok(metrics.listMinHeight>=0);await page.close()});

test('标签可跨资料改名合并和删除且不改变收藏置顶',async()=>{const page=await browser.newPage({viewport:{width:390,height:844}}),username=await register(page);await create(page,'网站',{'名称':'标签管理网站','网址':'https://manage-tags.example','说明':'','标签（逗号分隔）':'旧标签,合并目标'});await page.locator('.item-card',{hasText:'标签管理网站'}).click();await page.locator('#detail').getByRole('button',{name:'收藏'}).click();await page.locator('#detail').getByRole('button',{name:'置顶'}).click();await page.getByRole('button',{name:'返回'}).click();await page.getByRole('button',{name:'更多',exact:true}).click();await page.getByRole('menuitem',{name:'标签',exact:true}).click();await page.getByRole('button',{name:'管理标签 旧标签'}).click();const manage=page.getByRole('dialog',{name:'管理标签'});await manage.getByLabel('标签名称').fill('合并目标');await manage.getByRole('button',{name:'保存'}).click();await page.getByRole('dialog',{name:'标签'}).getByRole('button',{name:/合并目标.*1/}).click();await page.getByRole('dialog',{name:'标签：合并目标'}).getByRole('button',{name:/标签管理网站/}).click();await page.locator('#tag-results-dialog').waitFor({state:'hidden'});assert.equal(await page.locator('#detail').getByRole('button',{name:'取消收藏'}).count(),1);assert.equal(await page.locator('#detail').getByRole('button',{name:'取消置顶'}).count(),1);await page.getByRole('button',{name:'返回'}).click();await page.getByRole('button',{name:'更多',exact:true}).click();await page.getByRole('menuitem',{name:'标签',exact:true}).click();await page.getByRole('button',{name:'管理标签 合并目标'}).click();await page.getByRole('dialog',{name:'管理标签'}).getByRole('button',{name:'删除标签'}).click();await page.getByRole('dialog',{name:'删除标签？'}).getByRole('button',{name:'确认删除'}).click();await page.locator('#tag-delete-dialog').waitFor({state:'hidden'});await page.locator('#tag-manage-dialog').waitFor({state:'hidden'});assert.equal(await page.getByRole('dialog',{name:'标签'}).getByRole('button',{name:'管理标签 合并目标'}).count(),0);await page.reload();await page.getByLabel('用户名').fill(username);await page.getByLabel('主密码',{exact:true}).fill('correct horse battery staple');await page.getByRole('button',{name:'登录并解锁'}).click();await page.locator('#vault').waitFor({state:'visible'});await page.locator('nav').getByRole('button',{name:'网站',exact:true}).click();await page.locator('.item-card',{hasText:'标签管理网站'}).click();assert.equal(await page.locator('#detail').getByRole('button',{name:'取消收藏'}).count(),1);assert.equal(await page.locator('#detail').getByRole('button',{name:'取消置顶'}).count(),1);assert.equal(await page.locator('#detail').getByRole('button',{name:/标签 /}).count(),0);await page.close()});
...
test('独立收藏夹跨五类聚合且不改变置顶状态',async()=>{
 ...
 assert.equal(await detail.getByRole('button',{name:'置顶'}).count(),1,'收藏后仍应独立显示置顶');
 ...
 await detail.getByRole('button',{name:'置顶'}).click();await page.getByText('已置顶',{exact:true}).waitFor();assert.equal(await detail.getByRole('button',{name:'取消收藏'}).count(),1);
 await detail.getByRole('button',{name:'取消收藏'}).click();await page.getByText('已取消收藏',{exact:true}).waitFor();assert.equal(await detail.getByRole('button',{name:'取消置顶'}).count(),1,'取消收藏不得取消置顶');
...
test('三类字段隔离、当前分类搜索、编辑锁类型、危险区删除与备份', async()=>{
 ...
 await page.getByRole('button',{name:'更多',exact:true}).click();
 assert.equal(await page.getByRole('menuitem').count(),15);assert.equal(await page.getByRole('menuitem',{name:'全站搜索'}).count(),1);assert.equal(await page.getByRole('menuitem',{name:'自定义资料'}).count(),1);assert.equal(await page.getByRole('menuitem',{name:'标签',exact:true}).count(),1);assert.equal(await page.getByRole('menuitem',{name:'回收站'}).count(),1);assert.equal(await page.getByRole('menuitem',{name:'批量设置分组'}).count(),1);
...
test('安全中心异步加载前后保持固定窗口高度并仅内部滚动',async()=>{for(const engine of [chromium,webkit]){const own=engine===chromium?browser:await engine.launch({headless:true}),page=await own.newPage({viewport:{width:390,height:844}});try{await register(page);await page.getByRole('button',{name:'更多'}).click();await page.getByRole('menuitem',{name:'安全中心'}).click();const center=page.getByRole('dialog',{name:'安全中心'}),scroll=center.locator('.security-scroll');await center.waitFor({state:'visible'});const initial=await center.boundingBox();assert.ok(initial);await page.waitForFunction(()=>document.querySelector('#security-session-status')?.textContent==='');const loaded=await center.boundingBox();assert.ok(loaded);assert.ok(Math.abs(loaded.height-initial.height)<=1,`安全中心异步加载后高度跳变：${initial.height} -> ${loaded.height}`);const metrics=await center.evaluate(el=>{const scroll=el.querySelector('.security-scroll'),style=getComputedStyle(el);return{height:el.getBoundingClientRect().height,maxHeight:parseFloat(style.maxHeight),overflow:style.overflow,scrollOverflow:getComputedStyle(scroll).overflowY,scrollClient:scroll.clientHeight,scrollHeight:scroll.scrollHeight,viewport:window.innerHeight}});assert.ok(metrics.height<=metrics.viewport-15,JSON.stringify(metrics));assert.equal(metrics.overflow,'hidden');assert.equal(metrics.scrollOverflow,'auto');assert.ok(metrics.scrollClient<=metrics.scrollHeight);assert.ok(metrics.height>=500,'安全中心应在打开时预留稳定高度')}finally{await page.close();if(own!==browser)await own.close()}}});
