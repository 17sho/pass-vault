...
test('回收站彻底删除附件才调用 DELETE 并清除密文对象',async()=>{const page=await browser.newPage({viewport:{width:390,height:844}});await register(page);await page.getByRole('button',{name:'+ 新建'}).click();await page.locator('#picker').getByRole('button',{name:'附件',exact:true}).click();const upload=page.getByRole('dialog',{name:'上传附件'});await upload.getByLabel('选择文件').setInputFiles({name:'trash-proof.txt',mimeType:'text/plain',buffer:Buffer.from('recoverable encrypted attachment')});await upload.getByRole('button',{name:'加密并上传'}).click();await page.getByText('附件已上传',{exact:true}).waitFor();await page.getByRole('button',{name:'trash-proof.txt',exact:true}).click();await page.locator('#detail').getByRole('button',{name:'删除'}).click();await page.getByRole('button',{name:'移入回收站'}).click();await page.getByText('已移入回收站',{exact:true}).waitFor();let deletes=0;page.on('request',request=>{if(request.method()==='DELETE'&&request.url().includes('/api/attachments/'))deletes++});await page.getByRole('button',{name:'更多',exact:true}).click();await page.getByRole('menuitem',{name:'回收站'}).click();const trash=page.getByRole('dialog',{name:'回收站'}),row=trash.locator('.trash-item',{hasText:'trash-proof.txt'});await row.getByRole('button',{name:'彻底删除 trash-proof.txt'}).click();const confirm=page.getByRole('dialog',{name:'确认彻底删除'});assert.match(await confirm.textContent(),/无法撤销/);await confirm.getByRole('button',{name:'彻底删除'}).click();await page.getByText('已彻底删除',{exact:true}).waitFor();assert.equal(deletes,1);assert.equal(await row.count(),0);await page.close()});
