...
test('手机滚动列表菜单完整避开工具栏裁剪',async()=>{for(const engine of [chromium,webkit]){const own=engine===chromium?browser:await engine.launch({headless:true}),page=await own.newPage({viewport:{width:390,height:844}});try{await register(page);for(let i=0;i<8;i++)await create(page,'网站',{'名称':`菜单定位${i}`,'网址':`https://menu-${i}.example`,'说明':'','标签（逗号分隔）':''});const list=page.locator('#list');await list.evaluate(node=>{node.scrollTop=Math.max(0,node.scrollHeight-node.clientHeight)});const trigger=page.getByRole('button',{name:'菜单定位0的更多操作',exact:true});await trigger.click();const menu=trigger.locator('xpath=..').locator('.item-menu:not([hidden])');await menu.waitFor();const geometry=await menu.evaluate(node=>{const r=node.getBoundingClientRect(),list=document.querySelector('#list').getBoundingClientRect(),items=[...node.querySelectorAll('[role="menuitem"]')].map(item=>{const x=item.getBoundingClientRect(),hit=document.elementFromPoint(x.left+x.width/2,x.top+x.height/2);return{top:x.top,bottom:x.bottom,visible:hit===item||item.contains(hit)}});return{top:r.top,bottom:r.bottom,listTop:list.top,listBottom:list.bottom,items}});assert.ok(geometry.top>=geometry.listTop-1,JSON.stringify(geometry));assert.ok(geometry.bottom<=geometry.listBottom+1,JSON.stringify(geometry));assert.equal(geometry.items.length,4,JSON.stringify(geometry));assert.ok(geometry.items.every(item=>item.top>=geometry.listTop-1&&item.bottom<=geometry.listBottom+1&&item.visible),JSON.stringify(geometry));await menu.getByRole('menuitem',{name:'编辑'}).click();await page.locator('#editor').waitFor({state:'visible'})}finally{await page.close();if(own!==browser)await own.close()}}});
...
test('手机详情返回在结束帧完全透明，DOM替换不会产生可见跳变',async()=>{for(const engine of [chromium,webkit]){const own=engine===chromium?browser:await engine.launch({headless:true});const page=await own.newPage({viewport:{width:390,height:800}});try{await register(page);await create(page,'网站',{'名称':'返回透明网站','网址':'https://example.com','说明':'','标签（逗号分隔）':''});await page.locator('.item-card',{hasText:'返回透明网站'}).click();await page.locator('#detail.open').waitFor();await page.waitForTimeout(300);await page.evaluate(()=>{const d=document.querySelector('#detail');window.__exitOpacity=null;d.addEventListener('animationend',e=>{if(e.animationName==='mobile-detail-out')window.__exitOpacity=getComputedStyle(d).opacity},{once:true})});await page.locator('#detail .mobile-back').click();await page.waitForFunction(()=>window.__exitOpacity!==null);assert.equal(await page.evaluate(()=>window.__exitOpacity),'0','详情层必须在DOM清空前完全透明，避免WebKit在尾帧看到内容瞬间替换')}finally{await page.close();if(engine!==chromium)await own.close()}}});
