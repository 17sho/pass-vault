...
import {normalizeRecoveryRegistry,fieldDiff,encryptSharePackageV2,encryptShareObjectV2,decryptSharePackageV2,parseShareV2Fragment} from '../public/recovery-share-core.mjs';
...
for(const engine of [chromium,webkit])test(`${engine.name()} 回收站按字段结构遮挡秘密，不依赖可变标签`,async()=>{const browser=await engine.launch({headless:true}),page=await browser.newPage();try{await page.goto(base);const app=await readFile('public/app.mjs','utf8');assert.match(app,/field\.type==='secret'/);assert.match(app,/credential\?\.password/);assert.doesNotMatch(app,/const sensitive=\/\(密码\|密钥\|口令\|secret\|password\)\/i\.test\(label\)/)}finally{await browser.close()}});
...
for(const engine of [chromium,webkit])test(`${engine.name()} 匿名分享秘密生命周期、条件剪贴板清理与迟到附件隔离`,async()=>{
  const browser=await engine.launch({headless:true}),page=await browser.newPage();
  try{
    const token='L'.repeat(43),secret='Share-Secret-123',password='share-password',opaqueId='object_lifecycle',bytes=new TextEncoder().encode('attachment secret'),encrypted=await encryptSharePackageV2({version:2,records:[{type:'account',plain:{platform:'生命周期账号',credentials:[{username:'ordinary-user',password:secret}],notes:'',tags:[]}}],attachments:[{opaqueId,name:'secret.txt',mime:'text/plain',size:bytes.length,category:'other'}]},token,{password}),object=await encryptShareObjectV2(encrypted.packageKey,bytes,token,opaqueId);let releaseObject;const objectGate=new Promise(resolve=>{releaseObject=resolve});
    await page.addInitScript(()=>{let value='';Object.defineProperty(navigator,'clipboard',{configurable:true,value:{readText:async()=>value,writeText:async next=>{value=String(next)}}});window.__SHARE_CLIPBOARD_CLEAR_MS=40});
    await page.route('**/api/shares/claim',route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({envelope:encrypted.envelope})}));
    await page.route(`**/api/shares/objects/${opaqueId}`,async route=>{await objectGate;await route.fulfill({status:200,body:Buffer.from(object)})});
    await page.goto(`${base}/share#${token}.${encrypted.fragment}`);const passwordInput=page.locator('#share-unlock-password');assert.equal(await passwordInput.getAttribute('type'),'password');await passwordInput.fill(password);await page.locator('#share-unlock-form').dispatchEvent('submit');await page.getByText('生命周期账号',{exact:true}).waitFor();assert.equal(await passwordInput.inputValue(),'');assert.equal(await page.getByText(secret,{exact:true}).count(),0);
    const secretRow=page.locator('.share-field').filter({hasText:'密码 1'});await secretRow.getByRole('button',{name:'复制'}).click();assert.equal(await page.evaluate(()=>navigator.clipboard.readText()),secret);await page.waitForTimeout(80);assert.equal(await page.evaluate(()=>navigator.clipboard.readText()),'');
    await secretRow.getByRole('button',{name:'复制'}).click();await page.evaluate(()=>navigator.clipboard.writeText('new clipboard value'));await page.waitForTimeout(80);assert.equal(await page.evaluate(()=>navigator.clipboard.readText()),'new clipboard value');
    const userRow=page.locator('.share-field').filter({hasText:'账号 1'});await userRow.getByRole('button',{name:'复制'}).click();await page.waitForTimeout(80);assert.equal(await page.evaluate(()=>navigator.clipboard.readText()),'ordinary-user');
    await page.getByRole('button',{name:'解密并下载'}).click();await page.evaluate(()=>dispatchEvent(new Event('pagehide')));releaseObject();await page.waitForTimeout(100);assert.equal(await page.locator('#share-view-content').innerText(),'');assert.equal(await page.locator('a[download],img.share-attachment-preview').count(),0);assert.equal(await page.locator('#share-view-content').isHidden(),true);
  }finally{await browser.close()}
});

test('匿名分享生命周期静态门禁覆盖离页清理且不使用 Emoji',async()=>{const source=await readFile('public/share-view.mjs','utf8');assert.match(source,/pagehide/);assert.match(source,/beforeunload/);assert.match(source,/viewGeneration/);assert.doesNotMatch(source,/🔒|👁|📋|🔑|📎/)});
