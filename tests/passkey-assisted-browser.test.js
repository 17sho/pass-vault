...
test('真实WebAuthn注册和匿名断言创建会话、恢复vaultKey、拒绝challenge重放并支持撤销',async()=>{
 ...
 try{
  ...
  const authForm=page.locator('#auth-form');await authForm.getByLabel('用户名').fill('passkey-user');await authForm.getByLabel('主密码',{exact:true}).fill(password);await authForm.getByRole('button',{name:'登录并解锁'}).click();await created;await page.evaluate(()=>window.__lockVaultForTest());releaseLogin();await authForm.getByRole('button',{name:'登录并解锁'}).waitFor({state:'visible'});assert.equal((await json(page,'/api/session')).status,401);await page.unroute('**/api/login');
