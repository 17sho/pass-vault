...
const expected = new Map([
  ...
  ['stylesheet', `/style.css?v=${VERSION}-account-toggle`],
  ['app-shell', `/app-shell.css?v=${VERSION}`],
  ['module', `/app.mjs?v=${VERSION}-account-toggle`],
]);
...
test(`production HTML references current v${VERSION} frontend assets`, async () => {
  for (const path of ['public/index.html', 'dist/index.html']) {
    ...
    assert.doesNotMatch(html, new RegExp(`style\\.css\\?v=${VERSION.replaceAll('.', '\\.') }-(?!account-toggle(?:["']))`), path);
    assert.doesNotMatch(html, new RegExp(`app\\.mjs\\?v=${VERSION.replaceAll('.', '\\.') }-(?!account-toggle(?:["']))`), path);
