...
const expected = new Map([
  ...
  ['stylesheet', `/style.css?v=${VERSION}-bulk-group`],
  ['app-shell', `/app-shell.css?v=${VERSION}`],
  ['module', `/app.mjs?v=${VERSION}-bulk-group`],
]);
...
test(`production HTML references current v${VERSION} frontend assets`, async () => {
  for (const path of ['public/index.html', 'dist/index.html']) {
    ...
    assert.doesNotMatch(html, new RegExp(`style\\.css\\?v=${VERSION.replaceAll('.', '\\.') }-(?!bulk-group(?:["']))`), path);
    assert.doesNotMatch(html, new RegExp(`app\\.mjs\\?v=${VERSION.replaceAll('.', '\\.') }-(?!bulk-group(?:["']))`), path);
