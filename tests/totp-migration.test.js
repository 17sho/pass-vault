...
test('Linux TOTP table rebuild preserves an already-migrated revision column',()=>{const db=populated();db.exec('ALTER TABLE entries ADD COLUMN revision INTEGER NOT NULL DEFAULT 1 CHECK(revision>=1);UPDATE entries SET revision=7');assert.equal(migrateEntriesForTotp(db),true);assert.equal(db.prepare('SELECT revision FROM entries WHERE id=?').get('entry_123').revision,7);db.close()});
