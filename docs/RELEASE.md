...
4. 从该 tag 构建本次明确选择的 variant；v2.2.3 只生成 Cloudflare 的两个 archive 和 `SHA256SUMS`。
...
4. Build only the explicitly selected variant from that tag; v2.2.3 produces the two Cloudflare archives and `SHA256SUMS`.
