...
当前稳定版为v1.1.64。在Release页面下载Linux包与`SHA256SUMS`：
...
从旧版本升级到v1.1.64前，先做SQLite与附件目录的一致性备份，再安装到新的不可变版本目录并原子切换。本补丁不新增数据库迁移；早于v1.1.61的安装仍会在启动时执行既有的安全中心会话元数据幂等迁移，无需迁移密文或保险库密钥。

切换版本并重启后，确认日志无迁移错误且首页引用`app.mjs?v=1.1.64`；从第二个浏览器登录，确认安全中心显示其受信任IP、设备/浏览器类别和登录时间，再注销该会话并验证当前会话仍有效。
