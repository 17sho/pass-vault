...
> **Version note:** the Cloudflare archives attached to GitHub Release `v1.1.67` contain the complete code through that tag, including R2 lifecycle migrations `0011`–`0013`.
...
### 2.2 v1.1.67 Cloudflare artifacts
...
The check must report `OK`. The package contains the complete Cloudflare code through tag `v1.1.67`; still inspect and apply every pending migration during upgrades.
