...
> **Version note:** the Cloudflare archives attached to GitHub Release `v1.1.71` contain the complete code through that tag, including R2 lifecycle migrations `0011`–`0013`.
...
### 2.2 v1.1.71 Cloudflare artifacts

The Cloudflare assets in this Release are:

- `pass-vault-v2-cloudflare-1.1.71.tar.gz`
- `pass-vault-v2-cloudflare-1.1.71.zip`
- `SHA256SUMS`
...
The check must report `OK`. The package contains the complete Cloudflare code through tag `v1.1.71`; still inspect and apply every pending migration during upgrades.
