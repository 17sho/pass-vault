...
> **Version note:** the Cloudflare archives attached to GitHub Release `v2.2.2` contain the complete code through that tag, including R2 lifecycle migrations `0011`–`0013` and revision CAS/tombstone migrations `0014`–`0016`.
...
### 2.2 v2.2.2 Cloudflare artifacts
...
The check must report `OK`. The package contains the complete Cloudflare code through tag `v2.2.2`; still inspect and apply every pending migration during upgrades.
