...
- 当前`v1.1.66` Release只有Cloudflare tar/zip和`SHA256SUMS`；它是冻结tag制品，不含`main`后续`0011`–`0013` R2生命周期修复。
- 当前`v1.1.66`和`v1.1.65`均无Linux下载资产。Linux从审核过的当前`main`构建。
...
- 从审核过的当前`main`构建到新不可变版本目录；当前不存在v1.1.65/v1.1.66 Linux Release资产。
