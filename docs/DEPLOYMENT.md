...
- 当前`v1.1.68` Release提供隔离的Cloudflare与Linux tar/zip及`SHA256SUMS`，并包含截至tag的`0011`–`0013` R2生命周期修复。
- Linux可使用v1.1.68 Linux制品，或从同一tag/已审核main提交构建。
...
- 使用v1.1.68 Linux制品或从审核过的明确commit构建到新不可变版本目录。
