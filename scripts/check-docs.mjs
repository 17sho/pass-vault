...
const markdown = [...guides, 'release-notes-v1.1.58.md'];
...
for (const file of ['docs/cloudflare-deployment.zh-CN.md','docs/cloudflare-deployment.en.md']) {
  ...
  for (const required of ['v1.1.58','SHA256SUMS','apps/worker/migrations/','wrangler secret put INVITE_CODE','0005_invite_attempts.sql','0006_entries_created_at.sql','registration_unavailable','invalid_invite']) {
...
for (const file of ['docs/server-deployment.zh-CN.md','docs/server-deployment.en.md']) {
  const text = contents.get(file);
  for (const required of ['v1.1.58','SHA256SUMS','/etc/pass-vault-v2/pass-vault-v2.env','0600','systemctl restart pass-vault-v2','registration_unavailable']) {
    if (!text.includes(required)) throw new Error(`${file}: missing ${required}`);
  }
}
