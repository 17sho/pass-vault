...
const result={errors, type:await page.evaluate(()=>({type:window.document.querySelector('#search')?.placeholder, menuHidden:document.querySelector('#menu-panel')?.hidden, dialog:document.querySelector('dialog.custom-view-dialog')?.open||false, dialogText:document.querySelector('dialog.custom-view-dialog')?.innerText||'', nav:document.querySelector('nav')?.innerText, list:document.querySelector('#list')?.innerText})), customCount:await page.getByRole('button',{name:'自定义资料'}).count()}; console.log(JSON.stringify(result)); await browser.close();
