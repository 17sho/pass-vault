...
async function selectGroup(page, type, name) { const dialog = await openGroups(page, type); const choice = dialog.getByRole('button', { name: new RegExp(`^${name}（\\d+）$`) }); await choice.focus(); await choice.press('Enter'); await dialog.waitFor({ state: 'hidden' }); await expectActive(page, type, name); }
...
async function deleteCard(page, title) { const card = page.locator('.item-card').filter({ hasText: title }); await card.waitFor(); const more = card.getByRole('button', { name: /更多操作/ }); if (await more.count()) { await more.click(); await card.getByRole('menu').getByRole('menuitem', { name: '删除' }).click(); } else { await card.click(); await page.locator('#detail').waitFor({ state: 'visible' }); await page.locator('#detail').getByRole('button', { name: '删除', exact: true }).click(); } const confirm = page.getByRole('dialog', { name: /确认删除/ }); await confirm.getByRole('button', { name: /移入回收站|确认删除/ }).click(); await card.waitFor({ state: 'hidden' }); }
...
for (const base of sites) for (const [engineName, engine] of engines) {
  ...
  try {
    ...
    dialog = await openGroups(page, '附件'); await dialog.getByRole('button', { name: `删除 ${names.attachment}` }).click(); await page.getByRole('dialog', { name: '确认删除分组' }).getByRole('button', { name: '确认删除' }).click(); await page.getByText('分组已删除，资料已移至默认', { exact: true }).waitFor(); await dialog.getByRole('button', { name: '关闭' }).click(); await page.getByLabel('附件分类').selectOption('other'); await deleteCard(page, item.attachment); record.checkpoints.push('attachment-upload-group-move-category-delete');
